...
MinimalBrush::MinimalBrush(const MinimalProperties* properties, KoColorTransformation* transformation)
{
...
MinimalBrush::~MinimalBrush()
{
    delete m_transfo;
}


void MinimalBrush::paint(KritaPaintDeviceSP dev, qreal x, qreal y, const KoColor &color)
{
    m_inkColor = color;
    m_counter++;
    ...
    for (int by = -m_properties->radius; by <= m_properties->radius; by++) {
        ...
        for (int bx = -m_properties->radius; bx <= m_properties->radius; bx++) {
            // let's call that noise from ground to minimal :)
